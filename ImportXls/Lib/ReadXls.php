...
include_once "/home/webonise/projects/php/apps2013/testing_web/reader/Excel/reader.php";
class ReadXls {
    ...
    public function readExcelData(){
        ...
        //$excel->setInputEncoding('ISO-8859-7');
        ...
        if (empty($diff)) {
            ...
            for ($i = $dataStartsAt; $i <= $loop; $i++) {
                ...
                if(count($this->headers)==count($excel->sheets[0]['cells'][$i])){
                    $data[$j] = array_combine($this->headers, $excel->sheets[0]['cells'][$i]);
                }else{
                    foreach($this->headers as $key=>$header){
                        if(array_key_exists($key+1,$excel->sheets[0]['cells'][$i])){
